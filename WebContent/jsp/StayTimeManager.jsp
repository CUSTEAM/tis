...
<body>
	<form action="StayTimeManager" method="post" class="form-inline">
		<div class="bs-callout bs-callout-warning" id="callout-helper-pull-navbar">
			...
			<c:if test="${rule.cnt>0}"><p>截止後共修改 ${rule.cnt}次,修改次數上限由人事單位核定</p></c:if>
		</div>		
		...
		<div class="panel-heading">留校時間管理</div>
		...
		<table class="table table-bordered table-striped table-condensed">
			...
			<c:forEach begin="1" end="16" var="c">
				<tr>
					<td>${c}</td>
					<c:forEach begin="1" end="7" var="w">
						...
						<c:if test="${fk && !ck}"><input type="hidden" name="kind" ></input></c:if>	
						...
						<c:forEach items="${cs}" var="a">
							...
							<select style="width:100%;" class="form-control" disabled><option>${a.chi_name}</option></select><br>
							</c:if>
						</c:forEach>
						<c:forEach items="${st}" var="a">								
							...
							<select style="width:100%;" name="kind" class="form-control">
								<option value=""></option>
