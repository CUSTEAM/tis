...
輸入完成請先點選 
<button type="submit" class="btn btn-small btn-danger" <c:if test="${!empty date2}">disabled</c:if> name="method:save">儲存</button> 確認後再點選 <a class="btn btn-small" href="ScoreManager">離開</a>
<!-- a class="btn btn-small" onClick="javascript: return(confirm('請確認已點選儲存按鈕')); void('')" href="ScoreManager">離開</a-->
...
<table class="table table-striped table-hover" id="class${e.Oid}">
	<tr>
		...
		<td></td>
		<td></td>
		<td><div class="input-append"><input type="text" id="p1" name="p1" class="span1 editPro" <c:if test="${!empty edper}">readonly</c:if> value="${seldpro.score1}"/><span class="add-on">%</span></div></td>
		<td><div class="input-append"><input type="text" id="p2" name="p2" class="span1 editPro" <c:if test="${!empty edper}">readonly</c:if> value="${seldpro.score2}"/><span class="add-on">%</span></div></td>
		<td><div class="input-append"><input type="text" id="p3" name="p3" class="span1 editPro" <c:if test="${!empty edper}">readonly</c:if> value="${seldpro.score3}"/><span class="add-on">%</span></div>	
		</td>
		<td>
	...
	<tr>
		<td nowrap>學號</td>
		<td nowrap>姓名</td>
		<td nowrap>平時考  <button type="button" class="btn btn-small btn-success" onClick="showall();">其餘5次</button></td>
		<td nowrap>平時成績</td>
		<td nowrap>期中考</td>
		<td nowrap>期末考</td>
		<td nowrap>總成績</td>
	...
	<tr class="hairLineTdF">
		...
		<input type="text" name="score01"  id="score01${s.Oid}" value="${s.score01}" class="span1" onKeyUp="if(ck(this)){ck(this); km(event, 'score01${students[c.index+1].Oid}', 'score01${students[c.index-1].Oid}'); countScore('${s.Oid}')}"/>
		...
		<input type="text" name="score07"  id="score07${s.Oid}" value="${s.score07}" class="span1" onKeyUp="if(ck(this)){km(event, 'score07${students[c.index+1].Oid}', 'score07${students[c.index-1].Oid}'); countScore('${s.Oid}')}"/>
		...
		<input type="text" name="score10"  id="score10${s.Oid}" value="${s.score10}" class="span1" onKeyUp="if(ck(this)){km(event, 'score10${students[c.index+1].Oid}', 'score10${students[c.index-1].Oid}'); countScore('${s.Oid}')}"/>
		</span>
		</td>		
		<td><input type="text" name="score1"  id="score1${s.Oid}" value="${s.score1}" class="span1" onKeyUp="km(event, 'score1${students[c.index+1].Oid}', 'score1${students[c.index-1].Oid}'); if(ck(this)){finScore('${s.Oid}');}"/></td>
		<td><input type="text" name="score2" <c:if test="${!empty date1}">readonly</c:if> id="score2${s.Oid}" value="${s.score2}" class="span1"  <c:if test="${date1==null}"> onKeyUp="km(event, 'score2${students[c.index+1].Oid}', 'score2${students[c.index-1].Oid}'); if(ck(this)){finScore('${s.Oid}');}"</c:if>/></td>		
		<td><input type="text" name="score3"  <c:if test="${!empty date2}">readonly</c:if> id="score3${s.Oid}" value="${s.score3}" class="span1"  <c:if test="${date2==null}"> onKeyUp="km(event, 'score3${students[c.index+1].Oid}', 'score3${students[c.index-1].Oid}'); if(ck(this)){finScore('${s.Oid}');}"</c:if>/></td>		
		<td><input type="text" name="score"  <c:if test="${!empty date2}">readonly</c:if> id="score${s.Oid}" value="${s.score}" class="span1"  <c:if test="${date2==null}"> onKeyUp="ck(this),km(event, 'score${students[c.index+1].Oid}', 'score${students[c.index-1].Oid}');"</c:if> /></td>
	</tr>	
	</c:forEach>	
	<tr>
		<td colspan="100">
		<center>
		<button type="submit" class="btn btn-danger btn-large" <c:if test="${!empty date2}">disabled</c:if> name="method:save">儲存</button>
		<a class="btn btn-large" onClick="javascript: return(confirm('請確認已點選儲存按鈕')); void('')" href="ScoreManager">離開</a>
		</center>
		</td>
	</tr>
</table>
